...
def souris(event, x, y, flags, param):
    ...
    lo[0] = color - 5  # teinte légèrement plus basse
    hi[0] = color + 5  # teinte légèrement plus foncée


color = 100

# on peut baisser la saturation pour inclure plus de pixels
lo = np.array([color, 50, 50])
hi = np.array([color, 255, 255])
...
while True:
    ...
    # Dans cet ordre, on fait une fermeture morphologique
    # Dans l'autre sens, on fait une ouverture
    #mask=cv2.dilate(mask, None, iterations=1)
    #mask=cv2.erode(mask, None, iterations=1)

    # Permet une suppression du bruit correcte
    """ image=cv2.blur(image, (7, 7))
    mask=cv2.dilate(mask, None, iterations=4)
    mask=cv2.erode(mask, None, iterations=4) """

    image2 = cv2.bitwise_and(frame, frame, mask=mask)
    cv2.putText(frame, "Couleur: {:d}".format(
        color), (10, 30), cv2.FONT_HERSHEY_DUPLEX, 1, color_info, 1, cv2.LINE_AA)
    ...
    if len(elements) > 0:
        ...
        if rayon > 30:
            ...
            cv2.line(frame, (int(x), int(y)),
                     (int(x)+150, int(y)), color_info, 2)
            cv2.putText(frame, "Objet !!!", (int(x)+10, int(y) - 10),
                        cv2.FONT_HERSHEY_DUPLEX, 1, color_info, 1, cv2.LINE_AA)
